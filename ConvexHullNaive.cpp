...
// Takes O(n^3) time
vector<Point> convexHullNaive(const vector<Point>& points)
{
    vector<Point> output;
    auto n = points.size();
    ...
    do
    {
        output.push_back(points[p]);
        q = (p + 1) % n;

        for (int j = 0; j < n; ++j)
        {
            if (orientation(points[p], points[j], points[q]) == CCW)
            {
                q = j;                
            }
        }
...
TEST(convexHullNaive, simple)
{
    ...
    auto output1 = convexHullNaive(input1);

    EXPECT_TRUE(output1 == expected1);
}


TEST(convexHullNaive, complex)
{
    vector<Point> input = {
